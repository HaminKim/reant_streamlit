...
COLOR_TOP50_BAR = "#1f77b4"   # 인기 TOP50 바(단색)
COLOR_RANK_BAR  = "#1f77b4"   # 순위 바(단색)
...
with t_chart:
    ...
    if sel_disp == PLACEHOLDER or not sel_disp:
        ...
        if data.empty:
            ...
            <div class='kpi-wrap'>
              <div class='kpi'>
                <div class='kpi-label' style='color:{COLOR_BUY}'>총 매수(USD)</div>
                <div class='kpi-value'>{fmt_usd(total_buy)}</div>
              </div>
              <div class='kpi'>
                <div class='kpi-label' style='color:{COLOR_SELL}'>총 매도(USD)</div>
                <div class='kpi-value'>{fmt_usd(total_sell)}</div>
              </div>
              <div class='kpi'>
                <div class='kpi-label'>총 순매수(USD)</div>
                <div class='kpi-value'>{fmt_usd(total_net)}</div>
              </div>
              <div class='kpi'>
                <div class='kpi-label'>⚖️ 매수:매도 비율</div>
                <div class='kpi-value'>{(f"{ratio:.2f} : 1" if ratio else "-")}</div>
              </div>
            </div>
            """, unsafe_allow_html=True)
...
with t_top:
    ...
    if df_period.empty:
        ...
        chart_top = (
            alt.Chart(hits)
            .mark_bar(color=COLOR_TOP50_BAR)
            .encode(
...
with t_filter:
    ...
    if not trade_days:
        ...
        filtered = filtered.sort_values(by=["비율(BUY/SELL)", "최근20일_매수합"], ascending=[True, False])
...
with t_guide:
    ...
    <style>
      ...
      @media (prefers-color-scheme: dark) {{
        ...
        .guide-body {{ color: rgba(255,255,255,0.84); }}
      }}
    </style>
    """, unsafe_allow_html=True)
    ...
    with c1:
        ...
        <div class="guide-card">
          ...
          <div class="guide-body">
            한국 개인 투자자들의 <b>미국 주식 순매수·순매도 흐름</b>을 시각화한 도구입니다.<br/>
            특정 종목이 <b>언제, 얼마나, 어떤 방향으로</b> 매수·매도되는지 한눈에 볼 수 있어요.
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        <div class="guide-card">
          ...
          <div class="guide-body">
            주가는 펀더멘털만으로 움직이지 않습니다.<br/>
            관심, 유행, 공포와 기대 같은 <b>집단 심리</b>도 가격에 강하게 반영됩니다.<br/><br/>
            이 도구는 <b>주가 흐름과 투자자 심리(유행)의 상관관계</b>를 관찰하기 위해 만들었습니다.
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with c3:
        ...
        <div class="guide-card">
          ...
          <div class="guide-body">
            2024년 10월 이후<br/>
            <b>한국인 순매수·순매도 TOP50</b>에 한 번이라도 포함된 종목들만 대상으로 합니다.<br/><br/>
